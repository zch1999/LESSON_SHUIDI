        ...
        <ul>
          <li v-for="(item,index) in goods" class="menu-item" :key="index">
            <span class="text boder-1px">
              <span class="icon"></span>{{item.icon}}{{item.name}}
            </span>
          </li>
        </ul>
      </div>
      <div class="foods-wrappe" ref="foodsWrapper">
        <ul>
          <li v-for="item in goods" class="food-list" :key="item.id">
            <h1 class="title">{{item.name}}</h1>
            <ul>
              <li v-for="food in item.foods" class="food-item boder-1px" ref="foodList" :key="food.id">
                <div class="icon">
