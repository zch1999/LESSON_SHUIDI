...
//前序遍历5 2 7 6 4 8 9
func (node *Node)PreorderTraversal(n *Node){
	if(n !=nil){
		fmt.Printf("%d ",n.value)
		if(n.left != nil){
			node.PreorderTraversal(n.left)
		}
		if(n.right != nil){
			node.PreorderTraversal(n.right)
		}
	}
}

//中序遍历 2 6 7 5 8 4 9
func (node *Node)SequentialTraversal(n *Node){
	if(n !=nil){
		if(n.left != nil){
			node.SequentialTraversal(n.left)
		}
		fmt.Printf("%d ",n.value)
		if(n.right != nil){
			node.SequentialTraversal(n.right)
		}
	}
}

//后序遍历 6728945
func (node *Node)PostorderTraversal(n *Node){
	if(n !=nil){
		if(n.left != nil){
			node.PostorderTraversal(n.left)
		}
		if(n.right != nil){
			node.PostorderTraversal(n.right)
		}
		fmt.Printf("%d ",n.value)
	}
}
